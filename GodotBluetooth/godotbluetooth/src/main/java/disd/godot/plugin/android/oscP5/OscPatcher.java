...
public abstract class OscPatcher {
	...
	protected int parseBundle( Map m ) {
		...
		if ( bytes.length > OscBundle.BUNDLE_HEADER_SIZE ) {
			timetag = ( Long.valueOf( Bytes.toLong( Bytes.copy( bytes , 8 , 8 ) ) ) ).longValue( );
	...
	protected Object[] parseArguments( final byte[] theBytes ) {
		...
		while ( myTagIndex < _myTypetag.length ) {
			...
			case ( 0x63 ): // char c
				myArguments[ myTagIndex ] = ( Character.valueOf( ( char ) ( Bytes.toInt( Bytes.copy( theBytes , myIndex , 4 ) ) ) ) );
				myIndex += 4;
				break;
			case ( 0x69 ): // int i
				myArguments[ myTagIndex ] = ( Integer.valueOf( Bytes.toInt( Bytes.copy( theBytes , myIndex , 4 ) ) ) );
				myIndex += 4;
				break;
			case ( 0x66 ): // float f
				myArguments[ myTagIndex ] = ( Float.valueOf( Bytes.toFloat( Bytes.copy( theBytes , myIndex , 4 ) ) ) );
				myIndex += 4;

				break;
			case ( 0x6c ): // long l
			case ( 0x68 ): // long h
				myArguments[ myTagIndex ] = ( Long.valueOf( Bytes.toLong( Bytes.copy( theBytes , myIndex , 8 ) ) ) );
				myIndex += 8;
				break;
			case ( 0x64 ): // double d
				myArguments[ myTagIndex ] = ( Double.valueOf( Bytes.toDouble( Bytes.copy( theBytes , myIndex , 8 ) ) ) );
				myIndex += 8;
				break;
			//case ( 0x53 ): // Symbol S
			case ( 0x73 ): // String s
